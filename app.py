...
# ============================================================
# ROTA DE LOGIN (gera token JWT) - COM SUPORTE A OPTIONS
# ============================================================
...
def gerar_proposta_api():
    try:
        ...
        if not area or area == 0:
            AREA_POR_MODULO = 3.106352
            area = qtd_modulos * AREA_POR_MODULO
        ...
        # RECALCULA a economia mensal usando o primeiro ano (dados do frontend)
        if linhas_recebidas and len(linhas_recebidas) > 0:
            primeira_linha = linhas_recebidas[0]
            economia_mensal_calc = primeira_linha.get('economiaAnual', 0) / 12
            if economia_mensal_calc > 0:
                economia_mensal = economia_mensal_calc
...
O investimento total estimado para este projeto é de {format_moeda(investimento_ajustado)}, com uma economia mensal projetada para o primeiro ano de {economia_mensal_str}. O retorno do investimento (payback) está estimado em {payback_anos} anos e {payback_meses} meses, considerando a valorização da tarifa de energia e a degradação natural dos módulos ao longo dos anos.
